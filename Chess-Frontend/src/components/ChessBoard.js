              ...
              {[0, 1, 2, 3, 4, 5, 6, 7].map(r => [0, 1, 2, 3, 4, 5, 6, 7].map(c => {
                ...
                return (
                  <div key={`${r}-${c}`} onClick={() => onSquareClick(r, c)}
                    ...
                    style={{ width: `${SZ}px`, height: `${SZ}px`, background: getBg(r, c), display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", position: "relative", userSelect: "none" }}>
                    {isCk && (
                      <div style={{ position: "absolute", top: 3, right: 3, width: "13px", height: "13px", borderRadius: "50%", background: C.danger, boxShadow: "0 0 6px rgba(209,85,79,0.9)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "9px", color: "#fff", zIndex: 4 }}>✦</div>
                    )}
                    ...
                    {isT && !hasP && <div className="cm-hint-ring" style={{ width: "18px", height: "18px", borderRadius: "50%", background: "rgba(0,0,0,0.24)", boxShadow: "inset 0 1px 2px rgba(255,255,255,0.15)", position: "absolute", pointerEvents: "none" }} />}
                    {isT && hasP && <div className="cm-hint-ring" style={{ width: `${SZ - 6}px`, height: `${SZ - 6}px`, borderRadius: "50%", border: `4px solid ${C.danger}88`, position: "absolute", pointerEvents: "none" }} />}
                    {p && (
                      <span key={`${r}-${c}-${p}`} className="cm-piece" style={{ display: "inline-flex", transform: isSel ? "scale(1.08) translateY(-1px)" : "scale(1)", filter: isSel ? `drop-shadow(0 0 6px ${C.goldSoft})` : "none", transition: "transform var(--dur-fast) var(--ease)" }}>
                        <PieceGlyph piece={p} size={SZ * 0.8} />
                      </span>
                    )}
                  </div>
                );
